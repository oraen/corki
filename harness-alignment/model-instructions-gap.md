...
## 2026-09-22：旧缺基础记录的已提交模型结果优先回放

上一批的显式新宿主基础保护仍在 `_prepare_model_context` 过早触发：
旧无基础账本/无 checkpoint 的 Turn 已提交最终模型结果时，真实 Runtime
冷恢复先报 TurnFailed，未回放结果（1 个正例先红）。现在先检查该 Step
已提交/部分模型记录；有记录可进入既有回放，真正需要新模型采样仍在
`_call_model_owned` 前拒绝。Context 窗口自动压缩、降窗摘要与手动压缩也
在生成新摘要前检查同一来源；已安装摘要仍优先回放。小历史的已提交
结果冷恢复 TurnCompleted、零模型请求；大历史若需要新摘要，则安全地
TurnFailed、零模型请求且不安装压缩 marker，不把未来指令用于旧摘要。
取消意图的有效/损坏 10 组扩为有/无未来宿主覆盖共 20 例，仍先于采样
处理。相关恢复 12 文件 **256 passed / 17.00s**，4 workers/loadfile/
禁重启，退出 0。随后非 CLI 完整范围分组重跑：短期限敏感六文件
**459 passed / 63.84s**，其余文件 **15326 passed、7 skipped /
850.83s**，两批退出 0；独立收集 **15792 项**等于两批总数，
合计 **15785 passed、7 skipped**。七项跳过仍是六项缺失历史编译器、
一项文件系统非 UTF-8 名称限制。此结论仍不推断完全丢失的旧准入基础。

## 2026-09-22：旧 RUNNING Turn 缺基础指令时的新覆盖边界

Codex `session/world_state.rs::build_world_state_for_step` 从已接受的上一 Turn
设置或会话基础来源构造模型切换状态；Corki 的新 Turn 已将准入基础写入
`TurnRecord`，但旧 RUNNING 记录可能既无该字段也无可用 checkpoint。
在普通/手动压缩 × 非空/空原基础四个真实 Runtime/SQLite 反例中，冷开时
显式改为 `FUTURE BASE`，原实现均未拒绝，可能用新覆盖继续旧 Turn。
初版在 `resume_pending` 入口一律拒绝缺来源，扩大 12 文件产生 53 项失败，
包括已有摘要、取消和已提交结果的安全回放；移到模型/摘要新采样入口后仍有
34 项旧默认来源夹具受影响，不能把它们说成环境问题。最终只在冷开显式宿主
基础覆盖、且旧 Turn 的账本/checkpoint 均无法证明已准入基础时标记未知：
新采样或新摘要在副作用前 TurnFailed，零模型请求；已有压缩 marker 仍可
回放完成，取消和已提交结果不被拦下。原四例转绿，另补一例新覆盖下既有
摘要的正例；基础指令、模型设置、压缩及多种恢复 12 文件 **244 passed /
18.08s**，4 workers/loadfile/禁重启，退出 0；Ruff/diff 通过。
未显式覆盖的旧记录保留已有会话基础兼容行为；无任何可信来源且旧 Turn
曾用不可见的临时覆盖时，无法从历史反推其原指令，本项不宣称全域恢复。
此次生产修复后的非 CLI 完整范围分组重跑，短期限敏感六文件
**459 passed / 55.38s**、其余文件 **15314 passed、7 skipped / 728.54s**，
两批退出 0；独立收集 **15780 项**等于两批总数，合计 **15773 passed、
7 skipped**。七项跳过仍是六项缺失历史编译器及一项文件系统限制。
