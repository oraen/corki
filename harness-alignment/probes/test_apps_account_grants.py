"""Account grants and policy snapshots follow exact calls through the real Runtime."""
...
@pytest.mark.parametrize("case", ["same", "account", "connector", "cancel_review"])
def test_account_grants_and_policy_updates_preserve_call_authority(
    ...
    async def scenario():
        ...
        repeat = case in ("same", "account", "connector")
        ...
        class Model:
            ...
            async def stream(self, request):
                ...
                if self.steps == 1 and not nested:
                    ...
                    if case == "connector" and self.steps > first:
                        connector[0] = "next"
                        await runtime.refresh_mcp_apps_tools()
                    arguments = {
                        "link_id": "other"
                        if case == "account" and self.steps > first
                        else "account"
                    }
                    if nested:
                        call = ToolCall(
        ...
        expected_prompts = (
            0 if case == "late_disable" else 2 if case in ("account", "connector") else 1
        )
        expected_calls = 0 if case == "late_disable" else 2 if case == "same" else 1
        assert (len(prompts), len(calls)) == (expected_prompts, expected_calls)
        assert len(rows) == (2 if repeat else 1) and all(row[0] == "completed" for row in rows)
        errors = [json.loads(row[1])["is_error"] for row in rows]
        assert sum(errors) == int(case in ("late_disable", "account", "connector"))
        assert all(not json.loads(row[1]).get("dispatch_error", False) for row in rows)
        if case == "late_disable":
