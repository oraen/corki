"""Plugins feature gates native selected-root MCP contributions, not normal host MCP."""
...
def test_static_plugin_feature_filters_source_typed_mcp_before_search(
    tmp_path, monkeypatch, kind, mode, policy
):
    async def scenario():
            ...
            mcp_catalog=MCPCatalog(
                (
                    MCPRegistration(
                        ...
                        MCPCatalogSource(kind, None if kind == "config" else "fixture"),
                    ),
                )
            ),
        )
