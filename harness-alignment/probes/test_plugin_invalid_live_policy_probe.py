...
@pytest.mark.parametrize("declared", ["unrestricted", "allowlist", "disabled", "managed"])
def test_invalid_live_plugin_map_releases_selected_policy(
    tmp_path, monkeypatch, kind, mode, case, load_plugins, declared
):
    async def scenario():
        config = tmp_path / "config.toml"
        base_policy = '[plugins.fixture.mcp_servers.docs]\ndisabled_tools=["write"]\n'
        config.write_text(base_policy)
        ...
        try:
            ...
            await persistence.persist(
                ("codex_apps", "fixture", "Fixture", "read"),
                persistence.configuration,
                None,
            )
            assert "apps" in persistence.document, "valid raw user layer must publish"
