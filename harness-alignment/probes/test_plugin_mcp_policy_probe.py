"""Independent native-policy counterexamples, outside the frozen distribution."""
...
@pytest.mark.parametrize("policy", ["disabled", "allow", "deny", "control"])
def test_plugin_policy_filters_the_real_search_call_loop(tmp_path, monkeypatch, kind, mode, policy):
    async def scenario():
        ...
        assert calls == (["write"] if policy == "control" else []), (policy, calls)
        if policy == "disabled":
