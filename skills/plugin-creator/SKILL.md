---
name: plugin-creator
description: Create and scaffold Codex-compatible Corki plugin directories with a `.codex-plugin/plugin.json`, optional skills, Python tools, and MCP servers.
---

# Plugin Creator

> Corki compatibility: keep the Codex-compatible `.codex-plugin/plugin.json` format, but install
> local plugins under `~/.corki/plugins/<name>` (or a directory configured in
> `[plugins].directories`). Corki discovers these packages directly on its next process start;
> do not use Codex marketplace commands or `codex://` links for a Corki-only installation.
...
# By default creates in `${CORKI_HOME:-~/.corki}/plugins/<plugin-name>`.
...
For a Corki-only plugin, stop the installation flow here: Corki discovers that directory directly
on its next process start. Marketplace steps below apply only when the same package must also be
installed in Codex.
