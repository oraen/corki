...
Run the helper scripts with the `exec_command` tool from this skill's
...
Author the JSON spec with `apply_patch`, inspect script JSON output with
`exec_command` or directly from stdout.
