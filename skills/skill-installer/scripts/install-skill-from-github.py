#!/usr/bin/env python3
"""Install a skill from a GitHub repo path into $CORKI_HOME/skills."""
...
def _corki_home() -> str:
    return os.environ.get("CORKI_HOME", os.path.expanduser("~/.corki"))


def _tmp_root() -> str:
    base = os.path.join(tempfile.gettempdir(), "corki")
    os.makedirs(base, exist_ok=True)
    return base


def _request(url: str) -> bytes:
    return github_request(url, "corki-skill-install")
...
def _validate_skill(path: str, repo_root: str) -> None:
    ...
    for component in relative_path.split(os.path.sep):
        ...
        if os.path.islink(current_path):
            raise InstallError(
                "Symbolic links are not allowed in skills: "
                f"{os.path.relpath(current_path, repo_root)}"
            )
...
def _default_dest() -> str:
    return os.path.join(_corki_home(), "skills")
