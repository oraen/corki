---
name: skill-creator
description: Create or update a Corki skill with appropriately scoped instructions and any needed supporting resources.
metadata:
...
Create skills that give Corki useful, non-obvious guidance without constraining unrelated work.

## Core Principles

**Assume Corki is already capable.** Include only information that changes its decisions or improves its work. Remove generic advice, repeated instructions, speculative edge cases, and examples that do not materially clarify the task.
...
Ask clarifying questions only when the missing information matters and cannot be reasonably inferred. Respect a user-specified location; otherwise create discoverable skills in `$CORKI_HOME/skills`, or `~/.corki/skills` when `CORKI_HOME` is unset.
...
```bash
scripts/init_skill.py my-skill --path "${CORKI_HOME:-$HOME/.corki}/skills"
scripts/init_skill.py my-skill --path "${CORKI_HOME:-$HOME/.corki}/skills" --resources references
```
