---
name: "imagegen"
description: "Generate or edit raster images when the task benefits from AI-created bitmap visuals such as photos, illustrations, textures, sprites, mockups, or transparent-background cutouts. Use when Corki should create or transform a bitmap asset rather than repo-native code or vector."
---

# Image Generation Skill

> Corki compatibility: when an `image_gen` host tool is not registered, use the documented
> fallback CLI only after confirming that API use is intended. Use its `--out`/`--out-dir`
> arguments and place final project assets inside the workspace.
