...
| Read abstract | `curl -L "https://export.arxiv.org/api/query?id_list=2402.03300"` |
| Download full paper | `curl -L "https://arxiv.org/pdf/2402.03300" -o paper.pdf` |
...
```
# Abstract and metadata (Atom XML)
curl -L "https://export.arxiv.org/api/query?id_list=2402.03300"

# Full paper downloaded locally
curl -L "https://arxiv.org/pdf/2402.03300" -o paper.pdf
```

Use the bundled `pdf` skill to extract or render the downloaded paper. For a
cited research summary, combine this skill with `grounded-citations`.
...
3. **Read abstract**: fetch `https://export.arxiv.org/api/query?id_list=ID`
4. **Read full paper**: download `https://arxiv.org/pdf/ID`, then use the `pdf` skill
...
- For local PDF processing, use the bundled `pdf` skill
