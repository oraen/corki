...
Ledger path resolution (first wins):
  --ledger PATH
  $CORKI_CITATION_LEDGER
  $CORKI_HOME/cache/citations/ledger.json
"""
...
from _corki_home import get_corki_home  # noqa: E402
...
def resolve_ledger_path(explicit: str | None = None) -> Path:
    ...
    env = os.environ.get("CORKI_CITATION_LEDGER", "").strip()
    if env:
        return Path(env).expanduser()
    return get_corki_home() / "cache" / "citations" / "ledger.json"
...
def urls_from_json(payload: Any) -> list[tuple[str, str]]:
    ...
    Handles common web-tool JSON shapes and arbitrary nesting, in document
    order, deduped.
    """
...
# Retrieval tools often return markdown, so the most citation-worthy sentences
# are often the ones carrying inline links and emphasis around terms:
