...
Retrieval comes from configured MCP/plugin web tools, or `exec_command` (curl and other CLIs).

Ledger location: `$CORKI_HOME/cache/citations/ledger.json` (profile-aware).
Override per task with `--ledger <path>` or `CORKI_CITATION_LEDGER`.

## How to Run

```bash
S="<skill_dir>/scripts/sources.py"  # use skill_dir returned by skill_read
...
② **Register every source at retrieval time.** After each MCP/plugin web retrieval or `exec_command` fetch, pass the URLs to `sources.py add`
...
- **Citing a search snippet as if you read the page.** A search-result
  description supports only what it literally says. Fetch and inspect the page
  body before citing claims that depend on it.
...
- **Parallel subagents.** Each subagent has its own working directory; point
  them all at one ledger with `--ledger` (or `CORKI_CITATION_LEDGER`) if their
  outputs get merged, otherwise their ids will collide.
- **Quoting from a snippet instead of the page.** Evidence quotes must come
  from the extracted page text, not a search-result description. Fetch the
  page, save its text, then use `quote --from` with that file.
