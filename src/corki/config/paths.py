    ...
    def ensure_exists(self) -> None:
        ...
        if not self.config_file.exists():
            self.config_file.write_text(
                ...
                "[agent]\n"
                'model = "gpt-5"\n'
                "# Optional explicit per-turn limits; unset means no count ceiling.\n"
                "# max_steps = 24\n"
                "# max_tool_calls = 64\n"
                ...
                "[tools]\n"
                '# search_mode = "compatible"  # Harness-owned ordinary function tool search\n'
                "# command_timeout_seconds = 120  # optional lifetime cap; disabled by default\n"
                "command_yield_seconds = 10\n"
                "background_terminal_max_timeout = 300000  # empty-poll maximum in milliseconds\n"
                "output_char_budget = 20000\n\n"
                "[runtime]\n"
                "# Zero: unbounded output events. Positive values opt into UI backpressure.\n"
                "event_queue_size = 0\n\n"
                "[skills]\n"
                "enabled = true\n\n"
                "[plugins]\n"
                ...
                "disabled = []\n\n"
                "[realtime]\n"
                "# Local text steering while a CLI turn is running.\n"
                "enabled = true\n"
