...
- `raw_memories.md`
  ...
  - source of rollout-level metadata needed for MEMORY.md `### rollout_summary_files`
    annotations;
    you should be able to find `cwd`, `thread_id`, and `updated_at` there.
- `MEMORY.md`
...
The folder `#{memory_root}/` is a private snapshot prepared by Corki. Read
`#{phase2_workspace_diff_file}` in this same folder first. It contains the git-style diff from
the previous successful Phase 2 baseline to the current worktree. It is generated by Corki for
...
- <rollout_summaries/file1.md> (cwd=<path>, thread_id=<thread_id>, updated_at=<timestamp>, <optional status/usefulness note>)
...
- C) Provenance and metadata
  ...
  - Each rollout annotation must include `cwd=<path>`, `thread_id=<thread_id>`, and
...
- Typical workflows and tools (how they like to work, how they use Corki/agents, preferred formats)
