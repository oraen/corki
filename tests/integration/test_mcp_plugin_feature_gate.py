...
@pytest.mark.parametrize(
    "lifecycle", ["initial", "catalog", "reconcile", "refresh", "cold", "reload"]
)
    ...
    async def scenario():
        ...
        if lifecycle == "cold":
            ...
            if lifecycle == "reload":
                config.write_text(
                    "[features]\nplugins="
                    + ("true" if policy == "disabled" else "false")
                    + "\n"
                    + consent
                )
                ...
                }, "reload must actually publish"
                assert "apps" not in persistence.document
                assert runtime._settings.plugins_enabled is settings.plugins_enabled
            elif lifecycle == "catalog":
